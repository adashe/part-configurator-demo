class ValveAssembly {
    ...
    async getFilteredValveData(size, voltage) {
        ...
        let result = data.filter(
            (valve) => valve.valvePattern == size && valve.voltage == voltage
        );
    ...
    async updateStation(
        ...
        const valveArr = valveData.filter((valve) => valve.code == valveCode);
        const flowControlArr = flowControlData.filter(
            (flowControl) => flowControl.code == flowControlCode
        );
        const checkValveArr = checkValveData.filter(
            (checkValve) => checkValve.code == checkValveCode
        );
    ...
    countFlowControl() {
        ...
        stationsArr.forEach((station) => {
            if (
                station.flowControl != null &&
                station.flowControl.code != "0"
            ) {
                count++;
            }
        });
    ...
    calcCost() {
        ...
        stationsArr.forEach((station) => {
            if (station.valve != null) {
                cost +=
                    station.valve.cost +
                    station.flowControl.cost +
                    station.checkValve.cost;
            }
        });
